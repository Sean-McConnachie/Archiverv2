...
class logMsgsCog(commands.Cog, name='Logging module'):
    ...
    async def on_message(self, message: discord.Message):
        if not self.isActiveCategory(message):
            if message.channel.id == CONFIG["application"]["server_data"]["archiver_channel_id"]:
                ...
                await self.createFromSearch(message=message)
                return
            else:
                return
    ...
    async def createFromSearch(self, message: discord.Message):
        ...
        try:
            ...
            resp = await self.bot.db.fetch(query, topic_id, topic_name)
        ...
        if error is True:
            ...
            if len(archive_category.channels) >= 50:
                ...
                await self.bot.client.db.execute(query, None, None, None, old_channel.id)
                embed = prettyEmbed(
                ...
                await temp_msg.edit(embed=embed)
                await old_channel.delete(reason=f"The category was full. User {message.author.id} caused the overflow delete.")
            ...
            await self.bot.db.execute(query, new_channel.id, dt.datetime.now(), message.author.id,resp["topic_id"])
            ...
            temp = await self.bot.db.fetch(query, resp["topic_id"])
            old_messages = []
            json_messages = []
            senders = []
